...
const _BOLDYELLOW = '\x1B[1m\x1B[33m'
 ...
 */
export async function applyPresetCli(presetName: string, preset: PresetFile): Promise<void> {
  try {
...
export async function installPresetCli(
  ...
  try {
    ...
    if (source.startsWith('http://') || source.startsWith('https://')) {
      ...
      throw new Error(
        'URL installation is not supported. Please download the preset directory and install from local path.'
      )
      ...
      if (await isPresetInstalled(presetName)) {
        throw new Error(
          `Preset '${presetName}' is already installed. To reconfigure, use: ccr preset install ${presetName}\nTo delete and reinstall, use: ccr preset delete ${presetName}`
        )
      }
    } else {
