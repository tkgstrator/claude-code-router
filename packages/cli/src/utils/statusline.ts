...
import { CONFIG_FILE, getPresetDir, loadConfigFromManifest, readPresetFile } from '@ccr/shared'
...
async function getTokenSpeedStats(sessionId: string): Promise<{
  ...
  try {
    ...
    if (data.tokensPerSecond !== undefined && data.tokensPerSecond > 0) {
      ...
      const result = {
        tokensPerSecond: parseInt(data.tokensPerSecond, 10),
        timeToFirstToken: data.timeToFirstToken
      }
      return result
    }
...
async function getPresetThemeConfig(
  presetName: string
): Promise<{ theme: StatusLineThemeConfig | null; style: string }> {
  try {
...
export async function parseStatusLineData(input: StatusLineInput, presetName?: string): Promise<string> {
  try {
    ...
    const formattedTokenSpeed =
      tokenSpeedData && tokenSpeedData.tokensPerSecond > 0 ? tokenSpeedData.tokensPerSecond.toString() : ''
    ...
    const variables: Record<string, string> = {
      ...
      streamingIndicator,
      contextWindowSize:
        contextWindowSize > 1000 ? `${(contextWindowSize / 1000).toFixed(0)}k` : contextWindowSize.toString(),
      totalInputTokens:
        totalInputTokens > 1000 ? `${(totalInputTokens / 1000).toFixed(1)}k` : totalInputTokens.toString(),
      totalOutputTokens:
        totalOutputTokens > 1000 ? `${(totalOutputTokens / 1000).toFixed(1)}k` : totalOutputTokens.toString(),
      cost: formattedCost || '',
...
async function renderDefaultStyle(theme: StatusLineThemeConfig, variables: Record<string, string>): Promise<string> {
...
async function renderPowerlineStyle(theme: StatusLineThemeConfig, variables: Record<string, string>): Promise<string> {
