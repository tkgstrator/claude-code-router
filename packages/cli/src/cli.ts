...
import { getPresetDir, loadConfigFromManifest, PID_FILE, REFERENCE_COUNT_FILE, readPresetFile } from '@ccr/shared'
import { version } from '../package.json'
import { readConfigFile, restartService, run } from './utils'
import { activateCommand } from './utils/activateCommand'
import { executeCodeCommand, type PresetConfig } from './utils/codeCommand'
    ...
    if (manifest) {
      ...
      if (shouldStartServer && !isRunning) {
        ...
        if (await waitForService()) {
          ...
          console.error('Service startup timeout, please manually run `ccr start` to start the service')
          process.exit(1)
        }
      } else {
