...
import Server, { calculateTokenCount, type TokenizerService } from '@musistudio/llms'
...
export const createServer = async (config: any): Promise<any> => {
  ...
  app.post('/v1/messages/count_tokens', async (req: any, reply: any) => {
    ...
    if (model && model.includes(',') && tokenizerService) {
      try {
        ...
        const result = await tokenizerService.countTokens({ messages, system, tools }, tokenizerConfig)
  ...
  app.get('/api/transformers', async (req: any, reply: any) => {
    const transformers = (app as any)._server!.transformerService.getAllTransformers()
