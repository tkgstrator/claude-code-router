...
import type { ManifestFile, PresetFile, PresetInfo, PresetMetadata } from './types'
...
export async function extractPreset(sourceZip: string, targetDir: string): Promise<void> {
  ...
  if (entries.length > 0) {
    ...
    if (rootDirs.size === 1) {
      ...
      const hasManifestInRoot = entries.some(
        (e) => e.entryName === 'manifest.json' || e.entryName.startsWith(`${singleRoot}/manifest.json`)
      )
 ...
 */
const DYNAMIC_CONFIG_FIELDS = ['schema', 'template', 'configMappings']
