...
import { MergeStrategy, type ProviderConfig, type RouterConfig, type TransformerConfig } from './types'

/**
 * Merge Provider configuration
 * Overwrite if provider exists, otherwise add
 */
function mergeProviders(existing: ProviderConfig[], incoming: ProviderConfig[]): ProviderConfig[] {
...
export async function mergeConfig(
  ...
  const otherConfig = await mergeOtherConfig(result, presetConfig, strategy, callbacks?.onConfigConflict)
