...
export function generateEnvVarName(
  _fieldType: 'provider' | 'transformer' | 'global',
  entityName: string,
  fieldName: string
): string {
 ...
 */
function _extractEnvVarName(value: string): string | null {
 ...
 */
function sanitizeObject(config: any, path: string = '', sanitizedCount: number = 0): { sanitized: any; count: number } {
        ...
        for (let i = 0; i < pathParts.length; i++) {
          if (pathParts[i] === 'Providers' || pathParts[i] === 'transformers') {
            ...
            if (i + 1 < pathParts.length && pathParts[i + 1].match(/^\d+$/)) {
              ...
              const _parentPath = pathParts.slice(0, i + 2).join('.')
...
export function fillSensitiveInputs(config: any, inputs: Record<string, string>): any {
  ...
  function fillObject(obj: any, path: string = ''): any {
    ...
    if (Array.isArray(obj)) {
      return obj.map((item, index) => fillObject(item, path ? `${path}[${index}]` : `[${index}]`))
    }
