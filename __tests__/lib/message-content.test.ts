...
describe('normaliseContent', () => {
  ...
  test('classifies bare "[Request interrupted by user]" sentinel as system_text', () => {
    const blocks = normaliseContent([{ type: 'text', text: '[Request interrupted by user]' }])
    expect(blocks[0]?.kind).toBe('system_text')
  })

  test('classifies "[Request interrupted by user] + recap" combined block as system_text', () => {
    const combined = '[Request interrupted by user]\nThe user stepped away and is coming back. Recap in under 40 words.'
    const blocks = normaliseContent([{ type: 'text', text: combined }])
    expect(blocks[0]?.kind).toBe('system_text')
  })

  test('classifies the "[Request interrupted by user for tool use]" variant as system_text', () => {
    const blocks = normaliseContent([{ type: 'text', text: '[Request interrupted by user for tool use]' }])
    expect(blocks[0]?.kind).toBe('system_text')
  })

  test('strips <local-command-caveat> wrappers', () => {
    const blocks = normaliseContent([
      {
        type: 'text',
