...
// subscription smokes.
const hasCredentials = IS_REPLAY || (existsSync(AUTH_PATH) && !process.env.RIALTO_SKIP_LIVE_TESTS)
...
describe.skipIf(!hasCredentials)('codex subscription / all enabled models', () => {
  ...
  if (result.models.length === 0) {
    test('at least one enabled codex subscription model', () => {
      throw new Error('No enabled codex subscription models in /api/config — nothing to verify')
    })
    return
  }
