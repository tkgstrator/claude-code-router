...
import { type FastifyInstance, FastifyPluginAsync, type FastifyReply, type FastifyRequest } from 'fastify'
import { getPrismaClient } from '@/db/client'
import type { ConfigService } from '@/llms/services/config'
import type { ProviderService } from '@/llms/services/provider'
import type { TransformerService } from '@/llms/services/transformer'
import type { LLMProvider, RegisterProviderRequest } from '@/llms/types/llm'
...
export async function handleTransformerEndpoint(
  ...
  try {
    ...
    const { requestBody, config, bypass } = await processRequestTransformers(body, provider, transformer, req.headers, {
      req
    })

    // Send request to LLM provider
    const response = await sendRequestToProvider(requestBody, config, provider, fastify, bypass, transformer, {
      req
    })

    // Process response transformer chain
    const finalResponse = await processResponseTransformers(requestBody, response, provider, transformer, bypass, {
      req
    })
 ...
 */
async function processRequestTransformers(body: any, provider: any, transformer: any, headers: any, context: any) {
  ...
  if (!bypass && provider.transformer?.use?.length) {
    for (const providerTransformer of provider.transformer.use) {
      ...
      const transformIn = await providerTransformer.transformRequestIn(requestBody, provider, context)
      if (transformIn.body) {
  ...
  if (!bypass && provider.transformer?.[body.model]?.use?.length) {
    for (const modelTransformer of provider.transformer[body.model].use) {
      ...
      requestBody = await modelTransformer.transformRequestIn(requestBody, provider, context)
    }
  }
...
async function sendRequestToProvider(
  ...
  const reqLog = fastify.log && typeof fastify.log.child === 'function' ? fastify.log.child({ reqId }) : fastify.log
  ...
  reqLog.debug(
    {
      ...
      data: {
        ...
        bypass,
        messages: Array.isArray(requestBody?.messages) ? requestBody.messages.length : undefined,
        tools: Array.isArray(requestBody?.tools) ? requestBody.tools.length : undefined
      }
    },
    'llm request'
  )
  ...
  if (!response.ok) {
    ...
    const isSubscription = typeof transformer?.name === 'string' && transformer.name.endsWith('-oauth')
  ...
  reqLog.debug(baseLog, `${provider.name}/${requestBody?.model} ${response.status} ${durationMs}ms`)

  const usageProbe = typeof response.clone === 'function' ? response.clone() : null
  if (usageProbe) {
...
async function processResponseTransformers(
  ...
  if (!bypass && provider.transformer?.use?.length) {
    for (const providerTransformer of Array.from(provider.transformer.use).reverse() as Transformer[]) {
      if (!providerTransformer || typeof providerTransformer.transformResponseOut !== 'function') {
        continue
      }
      finalResponse = await providerTransformer.transformResponseOut!(finalResponse, context)
    }
  }

  // Execute model-specific response transformers
  if (!bypass && provider.transformer?.[requestBody.model]?.use?.length) {
    for (const modelTransformer of Array.from(provider.transformer[requestBody.model].use).reverse() as Transformer[]) {
      if (!modelTransformer || typeof modelTransformer.transformResponseOut !== 'function') {
        continue
      }
      finalResponse = await modelTransformer.transformResponseOut!(finalResponse, context)
    }
  }
...
export const registerApiRoutes = async (fastify: FastifyInstance) => {
  ...
  const transformersWithEndpoint = fastify.transformerService.getTransformersWithEndpoint()
  ...
  for (const [endPoint, transformersByName] of endpointTransformerMap) {
    ...
    fastify.post(endPoint, async (req: FastifyRequest, reply: FastifyReply) => {
  ...
  fastify.post(
    ...
    async (request: FastifyRequest<{ Body: RegisterProviderRequest }>, reply: FastifyReply) => {
      // Validation
      const { name, baseUrl, apiKey, models } = request.body

      if (!name?.trim()) {
        throw createApiError('Provider name is required', 400, 'invalid_request')
      }

      if (!baseUrl || !isValidUrl(baseUrl)) {
        throw createApiError('Valid base URL is required', 400, 'invalid_request')
      }
      ...
      if (!models || !Array.isArray(models) || models.length === 0) {
        throw createApiError('At least one model is required', 400, 'invalid_request')
      }

      // Check if provider already exists
      if (fastify.providerService.getProvider(request.body.name)) {
        throw createApiError(`Provider with name '${request.body.name}' already exists`, 400, 'provider_exists')
      }
  ...
  fastify.get(
    ...
    async (request: FastifyRequest<{ Params: { id: string } }>) => {
      const provider = fastify.providerService.getProvider(request.params.id)
      if (!provider) {
  ...
  fastify.put(
    ...
    async (
      ...
      const provider = fastify.providerService.updateProvider(request.params.id, request.body)
      if (!provider) {
        throw createApiError('Provider not found', 404, 'provider_not_found')
      }
      return provider
  ...
  fastify.delete(
    ...
    async (request: FastifyRequest<{ Params: { id: string } }>) => {
      const success = fastify.providerService.deleteProvider(request.params.id)
      if (!success) {
        throw createApiError('Provider not found', 404, 'provider_not_found')
      }
  ...
  fastify.patch(
    ...
    async (
      ...
      const success = fastify.providerService.toggleProvider(request.params.id, request.body.enabled)
      if (!success) {
        throw createApiError('Provider not found', 404, 'provider_not_found')
      }
      return {
        message: `Provider ${request.body.enabled ? 'enabled' : 'disabled'} successfully`
      }
    }
  )
}
