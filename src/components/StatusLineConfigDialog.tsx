...
export function StatusLineConfigDialog({ isOpen, onOpenChange }: StatusLineConfigDialogProps) {
  ...
  useEffect(() => {
    // Collect every hex background color in use across modules
    ...
    // Build CSS rules for each hex color
    let cssRules = ''
  ...
  const currentModules =
    currentThemeConfig && typeof currentThemeConfig === 'object' && 'modules' in currentThemeConfig
            ...
            <div className='border rounded-lg p-4 flex flex-col col-span-3'>
              ...
              <div
                ...
                className={`rounded bg-black/90 text-white font-mono text-sm overflow-x-auto flex items-center border border-border p-3 py-5 shadow-inner overflow-hidden ${
                  statusLineConfig.currentStyle === 'powerline' ? 'gap-0 h-8 p-0 items-center relative' : 'h-5'
                }`}
                ...
                onDrop={(e) => {
                  ...
                  if (moduleType) {
                    ...
                    // Pick default values based on the module type
              ...
              >
                {currentModules.length > 0 ? (
                  ...
                  <div className='flex flex-col items-center justify-center w-full py-4 text-center'>
                    ...
                    <span className='text-gray-500 text-sm'>{t('statusline.drag_hint')}</span>
                  </div>
                )}
              </div>
            </div>
                  ...
                  <div className='space-y-4'>
                    <div className='space-y-2'>
                      ...
                      <IconSearchInput
                        ...
                        onChange={(value) => handleModuleChange(selectedModuleIndex, 'icon', value)}
                    ...
                    {selectedModule.type === 'script' && (
                      <div className='space-y-2'>
                        ...
                        <Input
                          ...
                          onChange={(e) => handleModuleChange(selectedModuleIndex, 'scriptPath', e.target.value)}
                    ...
                    <Button variant='destructive' size='sm' onClick={deleteSelectedModule}>
                      {t('statusline.delete_module')}
                    </Button>
                  </div>
                ) : (
                  <div className='flex items-center justify-center h-full min-h-[200px]'>
                    <p className='text-muted-foreground text-sm'>{t('statusline.select_hint')}</p>
                  </div>
                )}
              </div>
            </div>
