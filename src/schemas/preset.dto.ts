 ...
 *
 * Moved from `src/shared/preset/types.ts` so that all Zod schemas in the
 * repo live under `src/schemas/*.dto.ts`. Free-form JSON values use the
 * recursive `JsonValue` schema instead of `z.any()` / `z.unknown()` so
 * every value still has a real, validatable type.
 *
 * Naming: the preset-specific Provider/Router/Transformer shapes are
 * exported as `PresetProviderSchema`, `PresetRouterConfigSchema`, and
 * `PresetTransformerConfigSchema` to avoid collision with the API-wire
 * shapes in `provider.dto.ts` / `router.dto.ts` / `transformer.dto.ts`.
 */
