...
// Prefix CCR stamps on thinking signatures it synthesises itself when an
// upstream produced reasoning without one (see the Gemini streaming
// converter). Those are placeholders that let a client pair text with
// reasoning locally — they are NOT Anthropic signatures and Anthropic
// cannot validate them.
const SYNTHETIC_SIGNATURE_PREFIX = 'ccr_'
...
export function keepSignedBlock(block: unknown): boolean {
  ...
  // A CCR-minted placeholder is as unusable to Anthropic as no signature
  // at all. Checking only for non-empty let these through, so a turn that
  // had been routed to a non-Anthropic provider poisoned every later
  // Anthropic turn in the same conversation with a signature the upstream
  // rejects — and the transcript replays it on every request, so the
  // failure is permanent for that conversation rather than transient.
  return !signature.startsWith(SYNTHETIC_SIGNATURE_PREFIX)
}
...
export class ClaudeCodeOauthTransformer extends OAuthTransformer {
  ...
  async auth(request: unknown, provider: RuntimeProvider, context: TransformerContext): Promise<TransformerAuthResult> {
    ...
    // a CCR-minted placeholder, neither of which Anthropic can validate,
