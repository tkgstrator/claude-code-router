...
import type { TokenizerService } from '../services/tokenizer'
import type { RouterRequest } from '../types/http'
import { sessionUsageCache, type Usage } from './cache'
...
const getUseModel = async (
  ...
  if (req.body.model?.includes('claude') && req.body.model?.includes('haiku') && globalRouter?.background) {
...
export const router = async (req: RouterRequest, _res: unknown, context: RouterContext): Promise<void> => {
...
const routerImpl = async (req: any, _res: any, context: RouterContext) => {
  ...
  if (rewritePrompt && system.length > 1 && system[1]?.text?.includes('<env>')) {
  ...
  try {
    ...
    const tokenizerConfig = context.tokenizerService?.getTokenizerConfigForModel(providerName, modelName)
    ...
    if (context.tokenizerService) {
      ...
      // Legacy fallback
      tokenCount = calculateTokenCount(messages as MessageParam[], system, tools as Tool[])
    }
